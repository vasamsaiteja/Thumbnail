...
class Gallery extends Component {
  state = {
    isClicked: imagesList[0].id,
  }

  viewImage = () => {
    const {isClicked} = this.state

    const filteredList = imagesList.filter(each => each.id === isClicked)
  ...
  render() {
    const filteredList = this.viewImage()
    const {isClicked} = this.state
          ...
          <ul className="gallery-list">
            {imagesList.map(eachThumbNail => (
              <ThumbnailItem
                ...
                isActive={isClicked === eachThumbNail.id}
